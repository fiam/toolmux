...
func CommandTree() actions.Spec {
	return group("notion", "Operate Notion pages and data sources",
		spec(ActionSearch, "search", ResourceWorkspace, actions.VerbSearch, actions.EffectRead, nil, []string{CapabilityReadContent}, actions.Use("search [query]"), actions.Short("Search Notion pages and data sources shared with Supacli"), actions.MaxArgs(1), stringFlag("query", "", "title query"), stringFlag("type", "all", "result type: all, page, data_source"), intFlag("limit", 20, "maximum results"), stringFlag("sort", "", "sort: edited, none"), stringFlag("direction", "desc", "sort direction: asc, desc")),
