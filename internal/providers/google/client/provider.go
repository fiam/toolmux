				...
				actions.Group("docs",
					...
					actions.Children(
						...
						googleDocsToolWithEffects("docs.insert_image", "insert-image", "Insert an inline image into a Google Docs document", actions.VerbUpdate, actions.EffectWrite, actions.EffectRead,
							...
							actions.StringFlag("name", "", "Drive file name when using --upload-file"),
							actions.StringFlag("mime-type", "", "image MIME type when using --upload-file"),
...
func driveToolDescription(name, fallback string) string {
	descriptions := map[string]string{
		...
		"upload":    "Upload a local file to Google Drive using Drive files.create multipart upload. With --make-public, Toolmux creates an anyone-reader permission on the uploaded file and returns a public Drive image URI that can be passed to google docs insert-image.",
...
func docsToolDescription(name, fallback string) string {
	descriptions := map[string]string{
		...
		"docs.insert_image":     "Insert an inline image into a Google Docs document using Docs batchUpdate insertInlineImage. Pass a public --uri, an already public --drive-file-id, or --upload-file with --make-public so Docs can fetch the image URL.",
