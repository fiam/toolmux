...
func (c Client) ListDriveFiles(ctx context.Context, query string, pageSize int, pageToken string) (DriveFilesResponse, error) {
	...
	values.Set("fields", "nextPageToken,files(id,name,mimeType,webViewLink,modifiedTime,trashed)")
...
func (c Client) GetDriveFile(ctx context.Context, fileID string) (DriveFile, error) {
	values := url.Values{}
	values.Set("fields", "id,name,mimeType,webViewLink,webContentLink,modifiedTime,trashed")
	var out DriveFile
...
func (c Client) CopyDriveFile(ctx context.Context, fileID string, options CopyDriveFileOptions) (DriveFile, error) {
	...
	if mimeType := strings.TrimSpace(options.TargetMIMEType); mimeType != "" {
...
func (c Client) UpdateDriveFile(ctx context.Context, fileID string, options UpdateDriveFileOptions) (DriveFile, error) {
	values := url.Values{}
	values.Set("fields", "id,name,mimeType,webViewLink,webContentLink,modifiedTime,trashed")
	values.Set("supportsAllDrives", "true")
	metadata := map[string]any{}
	...
	if options.Trashed != nil {
		metadata["trashed"] = *options.Trashed
	}
	var out DriveFile
	suffix := "/drive/v3/files/" + url.PathEscape(strings.TrimSpace(fileID))
	if options.Content == nil {
		if err := c.patchJSONQuery(ctx, suffix, values, metadata, &out); err != nil {
			return DriveFile{}, err
		}
		return out, nil
	}
	values.Set("uploadType", "multipart")
	if err := c.patchMultipartQuery(ctx, "/upload"+suffix, values, metadata, strings.TrimSpace(options.MIMEType), options.Content, &out); err != nil {
		return DriveFile{}, err
	}
	return out, nil
}
