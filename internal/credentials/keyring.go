...
func (s *KeyringStore) SaveOAuthTokens(ctx context.Context, ref ConnectionRef, tokens OAuthTokens) error {
	...
	if err := s.ring.Set(keyring.Item{
		Key:         key,
		Data:        data,
		Label:       fmt.Sprintf("Supacli %s OAuth tokens", normalizedRef.Provider),
...
func (s *KeyringStore) Doctor(ctx context.Context) Diagnostics {
	...
	probe := keyring.Item{
		Key:         key,
		Data:        []byte("ok"),
		Label:       "Supacli credential store probe",
		Description: "Temporary credential store probe created by supacli doctor",
	}
