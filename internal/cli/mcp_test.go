...
func TestMCPConfigureDryRunSupportsKnownAgents(t *testing.T) {
	...
	for _, want := range []string{
		...
		"claude: claude mcp add --scope project --transport stdio supacli-notion-read -- /opt/supacli/bin/supacli mcp serve --mcp-profile 'notion read' --tool 'notion.page.*' --exclude-tool '*.delete'",
		"gemini: gemini mcp add --scope project --transport stdio supacli-notion-read /opt/supacli/bin/supacli mcp serve --mcp-profile 'notion read' --tool 'notion.page.*' --exclude-tool '*.delete'",
	} {
...
func TestMCPEnableDryRunSupportsKnownAgents(t *testing.T) {
	...
	rendered := out.String()
	for _, want := range []string{
		"codex: codex mcp add supacli-notion-read -- /opt/supacli/bin/supacli mcp serve --mcp-profile 'notion read' --tool 'notion.*'",
		"claude: claude mcp add --scope project --transport stdio supacli-notion-read -- /opt/supacli/bin/supacli mcp serve --mcp-profile 'notion read' --tool 'notion.*'",
	} {
		if !strings.Contains(rendered, want) {
			t.Fatalf("expected dry-run output to contain %q, got:\n%s", want, rendered)
		}
...
func TestMCPDisableDryRunSupportsKnownAgents(t *testing.T) {
	...
	rendered := out.String()
	for _, want := range []string{
		"claude: claude mcp remove --scope local supacli-notion-read",
		"claude: claude mcp remove --scope user supacli-notion-read",
		"claude: claude mcp remove --scope project supacli-notion-read",
		"gemini: gemini mcp remove --scope user supacli-notion-read",
		"gemini: gemini mcp remove --scope project supacli-notion-read",
	} {
		if !strings.Contains(rendered, want) {
			t.Fatalf("expected dry-run output to contain %q, got:\n%s", want, rendered)
		}
...
func TestMCPProfileSetWritesLocalProfile(t *testing.T) {
	...
	dir := t.TempDir()
	profilePath := filepath.Join(dir, ".supacli", "config.yaml")
	profile := mcpProfileConfigFromSelection(mcpToolSelection{
		Tools:        []string{"notion.page.*"},
		ExcludeTools: []string{"*.delete"},
	})
	if err := writeSupacliConfigFile(profilePath, supacliConfigFile{
		Version: 1,
		MCP: mcpConfig{
...
func TestMCPProfilesLayerGlobalAndProjectDefaults(t *testing.T) {
	...
	if err := writeSupacliConfigFile(globalPath, supacliConfigFile{
		Version: 1,
		MCP: mcpConfig{
	...
	if err := writeSupacliConfigFile(filepath.Join(dir, ".supacli", "config.yaml"), supacliConfigFile{
		Version: 1,
		MCP: mcpConfig{
...
func TestRemoveMCPAgentsRemovesSupportedScopes(t *testing.T) {
	...
	for _, want := range []string{
		"claude mcp remove --scope local supacli",
		"claude mcp remove --scope user supacli",
		"claude mcp remove --scope project supacli",
		"gemini mcp remove --scope user supacli",
		"gemini mcp remove --scope project supacli",
	} {
...
func TestGeminiMCPConfigDetectsConfiguredServer(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(`{"mcpServers":{"supacli":{"command":"supacli"}}}`), 0o600); err != nil {
		t.Fatal(err)
	}
	if !mcpConfigHasServer(path, "supacli") {
		t.Fatal("expected supacli MCP server to be detected")
	}
...
func TestMCPAgentConfigureCommandsRejectInvalidScopes(t *testing.T) {
	...
	_, err = mcpAgentConfigureCommands("gemini", "supacli", mcpConfigureOptions{AgentScope: "local"}, []string{"mcp", "serve"})
