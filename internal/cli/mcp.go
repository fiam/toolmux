...
const supacliConfigRelPath = ".supacli/config.yaml"
...
func mcpEnableCommand(opts *options) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "enable [agent...]",
		Short: "Enable Supacli MCP for agents",
		Args:  cobra.ArbitraryArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
...
func mcpDisableCommand(opts *options) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "disable [agent...]",
		Short: "Disable Supacli MCP for agents",
		Args:  cobra.ArbitraryArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
...
func mcpProfileCommand(opts *options) *cobra.Command {
	...
	set := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			config, err := readSupacliConfigFile(configPath)
			...
			if err := writeSupacliConfigFile(configPath, config); err != nil {
				return err
			}
...
func mcpEnableSpec() actions.Spec {
	return actions.Command("supacli.mcp.enable", "enable",
		actions.Use("mcp enable [agent...]"),
		actions.Short("Enable Supacli MCP for agents"),
		actions.RBAC("agent_config", actions.VerbUpdate, actions.EffectNone, actions.EffectWrite),
		actions.Risks("agent-config"),
	)
}
...
func mcpDisableSpec() actions.Spec {
	return actions.Command("supacli.mcp.disable", "disable",
		actions.Use("mcp disable [agent...]"),
		actions.Short("Disable Supacli MCP for agents"),
		actions.RBAC("agent_config", actions.VerbUpdate, actions.EffectNone, actions.EffectWrite),
		actions.Risks("agent-config"),
	)
}
...
func resolveMCPToolSelectionFromDir(selection mcpToolSelection, startDir string) (mcpToolSelection, error) {
	globalPath, err := globalSupacliConfigPath()
	if err != nil {
...
func mcpProfileWritePath(scope mcpProfileScopeOptions) (string, string, error) {
	...
	if scope.Global {
		path, err := globalSupacliConfigPath()
		return path, "global", err
	}
	if path, ok, err := discoverSupacliConfigFile(""); err != nil {
		return "", "", err
	} else if ok {
		return path, "local", nil
	}
	return supacliConfigRelPath, "local", nil
}
...
type mcpProfileSource struct {
	Scope  string `json:"scope" yaml:"scope"`
	Path   string `json:"path" yaml:"path"`
	config supacliConfigFile
}

func loadMCPProfileSources(startDir, globalPath string) ([]mcpProfileSource, error) {
	var sources []mcpProfileSource
	if globalPath != "" {
		if config, ok, err := readSupacliConfigFileIfExists(globalPath); err != nil {
			return nil, err
		} else if ok {
			sources = append(sources, mcpProfileSource{Scope: "global", Path: globalPath, config: config})
		}
	}
	if localPath, ok, err := discoverSupacliConfigFile(startDir); err != nil {
		return nil, err
	} else if ok {
		config, err := readSupacliConfigFile(localPath)
		if err != nil {
			return nil, err
		}
...
func globalSupacliConfigPath() (string, error) {
	dir, err := os.UserConfigDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(dir, "supacli", "config.yaml"), nil
}

func discoverSupacliConfigFile(startDir string) (string, bool, error) {
	if startDir == "" {
		var err error
	...
	for {
		candidate := filepath.Join(dir, supacliConfigRelPath)
...
func readSupacliConfigFile(path string) (supacliConfigFile, error) {
	// #nosec G304 -- Supacli config paths are explicit local configuration.
	data, err := os.ReadFile(path)
	if err != nil {
		return supacliConfigFile{}, err
	}
	var config supacliConfigFile
...
func readSupacliConfigFileIfExists(path string) (supacliConfigFile, bool, error) {
	config, err := readSupacliConfigFile(path)
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return supacliConfigFile{}, false, nil
		}
		return supacliConfigFile{}, false, err
	}
	return config, true, nil
}

func writeSupacliConfigFile(path string, config supacliConfigFile) error {
	if config.Version == 0 {
		config.Version = 1
	}
	if config.MCP.Profiles == nil {
	...
	// #nosec G306 -- Supacli config is non-secret local tool configuration.
...
func selectMCPAgentsInteractive(cmd *cobra.Command, runtime mcpAgentRuntime, serverName string, configure mcpConfigureOptions) (mcpAgentInteractiveSelection, error) {
	...
	form := huh.NewForm(huh.NewGroup(
		huh.NewMultiSelect[string]().
			...
			Description("Selected agents will be updated to run `supacli mcp serve`.").
			Options(options...).
			Value(&selected).
			Height(height).
			Filterable(false),
