...
```text
https://api.supacli.com/oauth/notion/callback
```
...
`supacli connect notion` also prints the Notion redirect URI returned by
`supaclid`. If Notion shows `Missing or invalid redirect_uri`, copy that exact
URI into the Notion connection's redirect URI settings, or fix the server's
`SUPACLI_PUBLIC_URL` / `NOTION_REDIRECT_URI` environment variables.

It also writes local, ignored environment hints, including
`SUPACLI_SUPACLID_URL` and `SUPACLI_PUBLIC_URL`, to:
...
Redirect URI: https://api.supacli.com/oauth/notion/callback
```
