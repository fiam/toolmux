...
Supacli uses a split between the public OSS product repository and private
...
1. Signed `supacli` CLI binaries through GitHub Releases.
2. A Homebrew tap for the CLI.
3. Generic Ko-built `supaclid` Linux amd64/arm64 images through GHCR.
...
4. Terraform/Pulumi/CDK for Supacli's hosted production deployment.
5. Production rate-limit, abuse, billing, monitoring, or alerting internals.
6. Lambda-specific deployment wrappers that assume Supacli's AWS account.

## Private Infrastructure Repository

The private repo owns Supacli's hosted deployment.
...
5. Domain and certificate configuration for hosted `supaclid`.
...
```text
supacli CLI release -> Homebrew tap / GitHub Release
supaclid GHCR image -> private AWS deployment
```

The public `supaclid` image is a generic Linux HTTP server image published as
`ghcr.io/fiam/supaclid:<tag>` for amd64 and arm64. The private deployment may
...
Supacli's hosted deployment uses Supacli-owned provider apps and secrets from
private deployment infrastructure. Those secrets must never be committed to the
OSS repo.

## AWS Direction

Supacli's hosted deployment may use AWS Lambda, but that deployment is not part
of this OSS repo. The OSS repo should keep `supaclid` as a portable HTTP daemon
