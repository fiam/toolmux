...
leaf actions use the same metadata type. Root `add`, `connect`, `disconnect`,
`status [toolbox...]`, and `doctor [provider...]` remain explicit CLI commands
because their human interaction and diagnostics cross backend boundaries.
