...
Vault file:
  path: ~/.local/share/toolmux/vault.enc
...
5. Add `toolmux vault doctor` for keyring availability diagnostics.
...
2. Add `cmd/toolmux` and `cmd/auth-broker`.
...
broker_url: https://auth.toolmux.dev
profiles:
  default:
    output: table
    defaults:
