...
library, third-party, then `github.com/fiam/supacli` packages. Keep linter
...
11. GoReleaser snapshot release validation for the CLI archive matrix and
    Ko-built `supaclid` image manifest.
...
2. `.goreleaser.yaml` controls CLI archives for `supacli` and the Ko-built
   `supaclid` container image.
3. `supacli` release archives must cover macOS, Linux, and Windows on amd64
   and arm64 unless release support is intentionally changed and documented.
4. `supaclid` must not be released as a binary archive. Release it only as a
   Linux amd64/arm64 image at `ghcr.io/fiam/supaclid:<tag>`.
...
Provider integration tests that exercise the `supacli` command surface should
live with the provider package, usually as external tests such as
`internal/providers/notion/client` package `client_test`. Use
`internal/testutil/supaclitest` for command execution helpers instead of
creating provider-specific `runSupacli` wrappers.

Tests that need a real supaclid instance should use
`internal/testutil/supaclidtest` instead of constructing `server.NewHandler`
...
MCP support is exposed through `supacli mcp serve` over stdio. The MCP server
...
The CLI defaults to `https://api.supacli.com` for brokered OAuth flows. Use
`SUPACLI_SUPACLID_URL` for local development and self-hosted deployments. Do not
add provider-specific server URL flags unless the product contract changes.

`supaclid` exposes unauthenticated operational endpoints for deployment checks:
`GET /healthz` returns JSON health status, and `GET /build` returns Supacli
...
by default. Set `SUPACLI_TUNNEL_HOSTNAME` to use a stable Cloudflare account
...
`internal/providers/<provider>/broker` for supaclid OAuth/token broker support.
